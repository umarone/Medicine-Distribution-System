...
namespace Accounts.UI
{
    public partial class frmFindProductByBatchAndExpiry : MetroForm
    {
        ...
        private void grdFindStock_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Items.IdItem = Validation.GetSafeLong(grdFindStock.Rows[e.RowIndex].Cells[0].Value);
            Items.PackingSize = Validation.GetSafeString(grdFindStock.Rows[e.RowIndex].Cells[1].Value);
            Items.BatchNo = Validation.GetSafeString(grdFindStock.Rows[e.RowIndex].Cells[2].Value);
            Items.Mfg = Validation.GetSafeString(grdFindStock.Rows[e.RowIndex].Cells[3].Value);
            Items.Expiry = Validation.GetSafeString(grdFindStock.Rows[e.RowIndex].Cells[4].Value);
            Items.Qty = Validation.GetSafeLong(grdFindStock.Rows[e.RowIndex].Cells[5].Value);
        ...
        private void grdFindStock_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                if (grdFindStock.CurrentRow != null)
                {
                    ...
                    Items = new ItemsEL();  
                    Items.IdItem = Validation.GetSafeLong(grdFindStock.Rows[RowIndex].Cells[0].Value);
                    Items.PackingSize = Validation.GetSafeString(grdFindStock.Rows[RowIndex].Cells[1].Value);
                    Items.BatchNo = Validation.GetSafeString(grdFindStock.Rows[RowIndex].Cells[2].Value);
                    Items.Mfg = Validation.GetSafeString(grdFindStock.Rows[RowIndex].Cells[3].Value);
                    Items.Expiry = Validation.GetSafeString(grdFindStock.Rows[RowIndex].Cells[4].Value);
                    Items.Qty = Validation.GetSafeLong(grdFindStock.Rows[RowIndex].Cells[5].Value);

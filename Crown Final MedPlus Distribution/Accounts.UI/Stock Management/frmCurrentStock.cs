...
namespace Accounts.UI
{
    public partial class frmCurrentStock : MetroForm
    {
        ...
        private void FillGrid(List<ItemsEL> List)
        {
            ...
            if (List != null && List.Count > 0)
            {
                ...
                for (int i = 0; i < List.Count; i++)
                {
                    ...
                    grdCurrentStock.Rows[i].Cells[7].Value = List[i].Mfg;
                    ...
                    grdCurrentStock.Rows[i].Cells[16].Value = CommonFunctions.RemoveTrailingZeros(List[i].Qty);
                    grdCurrentStock.Rows[i].Cells[17].Value = CommonFunctions.RemoveTrailingZeros(List[i].Bonus);
                    grdCurrentStock.Rows[i].Cells[18].Value = CommonFunctions.RemoveTrailingZeros(List[i].UnitPrice);
                    grdCurrentStock.Rows[i].Cells[19].Value = CommonFunctions.RemoveTrailingZeros(List[i].TotalAmount);
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    else if (objActiveType.SoftwareType == "Medicine General Trading")
                    {
                        ...
                        grdCurrentStock.Columns["colItemName"].Width = 300;
                        grdCurrentStock.Columns["colpacking"].Width = 70;
                        grdCurrentStock.Columns["colBatchNo"].Width = 80;
                        grdCurrentStock.Columns["colMfg"].Width = 80;
                        grdCurrentStock.Columns["colExpiry"].Width = 80;
                        grdCurrentStock.Columns["colQty"].Width = 85;
                        grdCurrentStock.Columns["colBonusUnit"].Width = 85;
                        grdCurrentStock.Columns["colUnitPrice"].Width = 100;
                    }
                    else if (objActiveType.SoftwareType == "POS")
                    {
        ...
        private bool ValidateRows()
        {
            ...
            {
                if (grdCurrentStock.Rows[i].Cells["colBatchNo"].Value == null)
                {
                    MessageBox.Show("Batch No Is Missing in Row # " + i.ToString());
                    return false;
                }
                if (grdCurrentStock.Rows[i].Cells["colExpiry"].Value == null)
                {
                    MessageBox.Show("Expiry Is Missing in Row # " + i.ToString());
                    return false;
                }
            }
            return true;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (ValidateRows())
            {
                for (int i = 0; i < grdCurrentStock.Rows.Count - 1; i++)
                {
                    ...
                    oelCurrentStock.Mfg = Validation.GetSafeString(grdCurrentStock.Rows[i].Cells["colMfg"].Value);
        ...
        void frmFindItems_ExecuteFindPorudctsEvent(object Sender, ItemsEL oelItems)
        {
            ...
            //if (manager.VerifyAccount(Operations.IdCompany, "Items", oelItems.AccountNo).Count > 0)
            {
                ...
                //    {
                //        if (oelItems.IdItem == Validation.GetSafeLong(grdCurrentStock.Rows[i].Cells["colIdItem"].Value))
                //        {
                //            MessageBox.Show("Product Already exists In Grid List...");
                //            return;
                //        }
                //    }
                //}
                ...
                List<ItemsEL> listItem = manager.GetItemsCurrentStockById(oelItems.IdItem, Operations.IdProject);
                if (listItem.Count > 0)
                {
                    FillGrid(listItem);  
                }               
        ...
        private void grdCurrentStock_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 16)
            {
                if (grdCurrentStock.Rows[e.RowIndex].Cells["colUnitPrice"].Value == null)
                {

    ...
    public partial class frmSalesReturn : MetroForm
    {
            ...
            if (list.Count > 0)
            {
                ...
                if (objActiveType != null)
                {
                    ...
                    else if (objActiveType.SoftwareType == "Medicine General Trading")
                    {
                        ...
                        DgvStockSales.Columns["colItemName"].Width = 330;
                        DgvStockSales.Columns["colpacking"].Width = 100;
                        DgvStockSales.Columns["colQty"].Width = 100;
                        DgvStockSales.Columns["colUnitPrice"].Width = 100;
                    }
        ...
        private bool ValidateRows()
        {
            ...
            for (int i = 0; i < DgvStockSales.Rows.Count - 1; i++)
            {
                ...
                else if (DgvStockSales.Rows[i].Cells["colBatchNo"].Value == null)
                {
                    MessageBox.Show("Batch No Is Compulsory....");
                    return false;
                }
                else if (DgvStockSales.Rows[i].Cells["colExpiry"].Value == null)
                {
                    MessageBox.Show("Expiry Is Missing....");
                    return false;
                }
                ...
                if (ValidateControls())
                {
                    if (ValidateBookPeriod())
                    {
                        ...
                        if (!IsNetTransaction)
                        {
                            ...
                            if (!IdVoucher.HasValue)
                            {
                                oelCustomerTransaction.CreatedDateTime = null;
                            }
                            else
                            {
                                if (VoucherPreviousBillAmount == Validation.GetSafeDecimal(txtTotalAmount.Text) && VoucherDateTime == null)
                        ...
                        if (IsNetTransaction)
                        {
                            ...
                            if (!IdVoucher.HasValue)
                            {
                                oelCashTransaction.CreatedDateTime = null;
                            }
                            else
                            {
                                if (VoucherPreviousBillAmount == Validation.GetSafeDecimal(txtTotalAmount.Text) && VoucherDateTime == null)
                        ...
                        if (!IdVoucher.HasValue)
                        {
                            oelSaleTransaction.CreatedDateTime = null;
                        }
                        else
                        {
                            if (VoucherPreviousBillAmount == Validation.GetSafeDecimal(txtTotalAmount.Text) && VoucherDateTime == null)
                        ...
                        for (int j = 0; j < DgvSalesTransactions.Rows.Count - 1; j++)
                        {
                            ...
                            if (!IdVoucher.HasValue)
                            {
                                oelVoucherDetail.CreatedDateTime = null;
                            }
                            else
                            {
                                if (VoucherPreviousBillAmount == Validation.GetSafeDecimal(txtTotalAmount.Text) && VoucherDateTime == null)
        ...
        private VoucherDetailEL CreateInventoryTransaction(VouchersEL oelVoucher, List<ItemsEL> EachItem, decimal Quantity, string BatchNo)
        {
            ...
            oelInventoryVoucherDetail.Debit = Validation.GetSafeDecimal(manager.GetItemsValueBatchWise(EachItem[0].IdItem.Value, BatchNo)) * Quantity; //EachItem[0].AVGEvaluationUnitPrice);
            ...
            if (!IdVoucher.HasValue)
            {
                oelInventoryVoucherDetail.CreatedDateTime = null;
            }
            else
            {
                if (VoucherPreviousBillAmount == Validation.GetSafeDecimal(txtTotalAmount.Text) && VoucherDateTime == null)
        ...
        private VoucherDetailEL CreateCOGSTransaction(VouchersEL oelVoucher, List<ItemsEL> EachItem, decimal Quantity, string BatchNo)
        {
            ...
            oelCOGSVoucherDetail.Credit = Validation.GetSafeDecimal(manager.GetItemsValueBatchWise(EachItem[0].IdItem.Value, BatchNo)) * Quantity;//EachItem[0].AVGEvaluationUnitPrice);
            ...
            if (!IdVoucher.HasValue)
            {
                oelCOGSVoucherDetail.CreatedDateTime = null;
            }
            else
            {
                if (VoucherPreviousBillAmount == Validation.GetSafeDecimal(txtTotalAmount.Text) && VoucherDateTime == null)
